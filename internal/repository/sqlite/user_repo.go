...
func NewUserRepo(cfg config.SqliteConfig) (*UserRepo, error) {
	dbx, err := NewDb(cfg)
	if err != nil {
		return nil, err
	}

	return NewUserRepoWithDB(dbx), nil
}

func NewUserRepoWithDB(db *sqlx.DB) *UserRepo {
...
func (r *UserRepo) UserCount(ctx context.Context) (int, error) {
	...
	if err != nil {
		return count, fmt.Errorf("db.NamedExecContext() err: %w", err)
	}
	return count, nil
}

func (r *UserRepo) SaveIncome(ctx context.Context, income model.IncomeRequest) (model.IncomeRequest, error) {
...
func (r *UserRepo) SetUserIsManager(ctx context.Context, userID int64, isManager bool) error {
