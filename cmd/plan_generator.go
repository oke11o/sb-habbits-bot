...
func main() {
	...
	sectionsPerDay := 3  // Количество секций в день

	// Шаг 3: Генерация плана
	generatedPlan := generateUniquePlanForDays(plan, sectionsToInclude, tasksPerSection, sectionsPerDay, days)
...
func generateUniquePlanForDays(plan StudyPlan, sections []string, tasksPerSection, sectionsPerDay, days int) []map[string][]string {
	...
	for day := 0; day < days; day++ {
		...
		// Случайный выбор секций
		selectedSections := selectRandomSections(sections, sectionsPerDay)
...
// Функция для случайного выбора секций
func selectRandomSections(sections []string, count int) []string {
	rand.Seed(time.Now().UnixNano())
	rand.Shuffle(len(sections), func(i, j int) { sections[i], sections[j] = sections[j], sections[i] })
	return sections[:min(count, len(sections))]
}
